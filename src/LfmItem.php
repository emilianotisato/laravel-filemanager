...
class LfmItem
{
    ...
    public function __get($var_name)
    {
        if (!array_key_exists($var_name, $this->attributes)) {
            $function_name = class_exists('Str') ? Str::camel($var_name) : camel_case($var_name);
    ...
    {
        if (!$this->isDirectory()) {
            return class_exists('Str') ? Str::startsWith($this->mimeType(), 'image') : starts_with($this->mimeType(), 'image');
        }
